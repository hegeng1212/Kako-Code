...
function renderPlanToolEntry(
  ...
  out.push({
    text: indent(renderPlanPreviewHint(), BODY_START + 2),
  });
...
function renderActivityGroup(
  ...
  if (expanded) {
    for (const entry of entries) {
      ...
      out.push({
        text: indent(renderToolInvocationLine(entry), BODY_START + 2),
      });
      for (const line of renderToolOutputLines(entry, width, BODY_START + 2)) {
        out.push({ text: indent(line, BODY_START + 4) });
      }
    }
  }
  out.push(...timelineBlockGaps());
...
function renderChoiceEntry(turn: ChatTurn, entry: ChoiceTimelineEntry): RenderLine[] {
  ...
  if (expanded) {
    for (let i = 0; i < entry.options.length; i++) {
      ...
      out.push({
        text: indent(renderChoiceOptionLine(opt, i, entry.multiSelect), BODY_START + 2),
      });
    }
  }
  out.push(...timelineBlockGaps());
...
export function renderTipLine(tip: string): string {
  ...
  return indent(`${ansi.muted}└ Tip: ${body}${ansi.reset}`, LINE_INDENT);
}
