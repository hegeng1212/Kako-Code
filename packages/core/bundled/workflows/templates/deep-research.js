...
// ─── Search: parallel WebSearch per angle ───
phase("Search")
log("Search: " + scope.angles.length + " angles")
...
// ─── Fetch: URL-dedup then parallel fetch+extract ───
phase("Fetch")
const fetchJobs = []
for (const searchResult of searchResults) {
  ...
  for (const source of sorted) {
    ...
    if (seen.has(key)) {
      dupes.push({ ...source, angle: searchResult.angle, dupOf: seen.get(key) })
      continue
    }
    if (fetchSlots <= 0 && relRank[source.relevance] >= 1) {
      budgetDropped.push({ ...source, angle: searchResult.angle })
      continue
    }
    seen.set(key, { angle: searchResult.angle, title: source.title })
    fetchSlots--
    fetchJobs.push({ source, angle: searchResult.angle })
  }
}
log("Fetch: " + fetchJobs.length + " novel URLs (" + dupes.length + " dupes, " + budgetDropped.length + " budget-dropped)")

const allSources = (await parallel(
  fetchJobs.map(({ source, angle }) => () =>
    agent(FETCH_PROMPT(source, angle), {
